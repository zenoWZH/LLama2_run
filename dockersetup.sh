...
docker exec -d inference_benchmark /bin/bash -c "cd ./LLama2_run && git pull origin master"
docker exec -it inference_benchmark /bin/bash -c "cd ./LLama2_run && poetry run python main.py"